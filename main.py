    ...
    for key in gemini_rotator.get_order():
        try:
            ...
            if not uploaded_file_name:
                raise RuntimeError("Uploaded file name missing from response.")
...
async def handle_media(client, message: Message):
    ...
    if FFMPEG_BINARY is None:
        ...
        if file_ext and file_ext not in gemini_supported_extensions:
             await message.reply_text("⚠️ FFMPEG ma la heli karo. Ma beddeli karo Videos-ka ama format-yada aan la taageerin. Transcription-ka wuu fashilmayaa.")
             return
