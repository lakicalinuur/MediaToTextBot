...
import logging
import time
import mimetypes  # Added to detect file types automatically
...
# REMOVED: FFMPEG_BINARY definitions
...
# REMOVED: convert_to_wav function
...
def upload_and_transcribe_gemini(file_path: str, uid: int) -> str:
    # UPDATED: No conversion, no extension check. Uploads "as is".
    file_size = os.path.getsize(file_path)
    
    # Guess mime type based on the file extension
    mime_type, _ = mimetypes.guess_type(file_path)
    if not mime_type:
        # Default fallback if unknown, Gemini usually handles audio/mp3 broadly
        mime_type = "audio/mp3" 
    ...
    try:
        ...
        prompt = "Transcribe the audio/video in this file. Automatically detect the language and provide a clean transcription. Do not add intro phrases."
        ...
        if uploaded_name:
            try:
                ...
                pass
        # No converted file cleanup needed
...
def handle_media(message):
    ...
    try:
        # UPDATED: Get extension from Telegram to ensure MimeType works
        file_info = bot.get_file(media.file_id)
        ext = os.path.splitext(file_info.file_path)[1] # Extracts .mp3, .oga, etc.
