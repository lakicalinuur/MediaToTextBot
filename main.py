...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        if msg == "API_KEY_MISSING":
            try:
                bot.send_message(call.from_user.id, "Please send your Gemini API key first. To transcribe a video from channel @NotifyB forward the channel post to me in this private chat or use /setkey")
            except:
                bot.send_message(chat_id, "Please send your Gemini API key first. Use /setkey in private chat.")
    ...
    try:
        ...
        try:
            ...
            if em == "API_KEY_MISSING":
                ...
                if forwarded_from and getattr(forwarded_from, 'username', None) and forwarded_from.username.lower().lstrip("@") == "notifyb":
                    try:
                        bot.send_message(message.from_user.id, "Please send your Gemini API key first. Forward the channel post from @NotifyB to me in this private chat after you set your key, or use /setkey to paste your key now.")
                    except:
                        bot.reply_to(message, "Please send your Gemini API key first. Use /setkey in a private chat with me.")
                else:
                    try:
                        bot.send_message(message.from_user.id, "Please send your Gemini API key first. Use /setkey in a private chat with me or paste your key here.")
                    except:
