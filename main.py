...
async def removekey_cmd(client, message):
    ...
    if users_col is not None and result.deleted_count > 0:
...
async def process_text_action(client, call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        loop = get_event_loop()
        res = await loop.run_in_executor(None, ask_gemini, text, prompt_instr, call.from_user.id)
...
async def handle_media(client, message):
    ...
    # Kordhinta Chat Action si loo yareeyo daahida la dareemo
    if message.audio or message.voice:
        initial_action = enums.ChatAction.UPLOAD_AUDIO
    else:
        initial_action = enums.ChatAction.UPLOAD_VIDEO
        
    await client.send_chat_action(message.chat.id, initial_action)
    
    if getattr(media, 'file_size', 0) > MAX_UPLOAD_SIZE:
        await message.reply_text(f"Just Send me a file less than {MAX_UPLOAD_MB}MB 😎")
        return
        
    initial_msg = await message.reply_text("⏳ Waan helay faylkaaga. Hadda waxaan ku shaqeynayaa Transcription-ka...")
    
    dl_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.id}_{media.file_unique_id}")
    
    try:
        # Tallaabada 1: Soo dejinta Faylka (Downloading)
        await client.send_chat_action(message.chat.id, enums.ChatAction.DOWNLOADING)
        await client.download_media(message, file_name=dl_path)
        
        # Tallaabada 2: Habaynta/U diridda Gemini (Processing/API Call)
        await client.edit_message_text(message.chat.id, initial_msg.id, "🚀 Faylka waa la helay. Hadda Transcription-ka Gemini ayaan u dirayaa...")
        await client.send_chat_action(message.chat.id, enums.ChatAction.TYPING)
        
        loop = get_event_loop()
        text = await loop.run_in_executor(None, upload_and_transcribe_gemini, dl_path, message.from_user.id)
        
        if not text:
            raise ValueError("Empty response")
            
        # Tallaabada 3: Dirista Jawaabta (Sending Response)
        ...
        if em == "API_KEY_MISSING":
            await client.edit_message_text(message.chat.id, initial_msg.id, "❌ API Key waa maqan yahay. Fadlan soo dir furahaaga Gemini.")
            await send_key_missing_alert(client, message.chat.id)
        elif em.startswith("API_DAILY_LIMIT_REACHED"):
            parts = em.split("|")
            secs = int(parts[1]) if len(parts) > 1 else seconds_left_for_user(message.from_user.id)
            await client.edit_message_text(message.chat.id, initial_msg.id, f"❌ Waa la gaaray xadka maalmeed. Hadda: {format_hms(secs)}.")
        else:
            await client.edit_message_text(message.chat.id, initial_msg.id, f"❌ Khalad dhacay: {e}")
